...
public class StepOverHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (nextIndex == parentItem.getItemCount()) { // stepped out from last subelement, indexOutOfBounds
			viewer.setSelection(new StructuredSelection(parentItem.getData()));
			parentItem.setExpanded(false);
			return null;
		}
		
		viewer.setSelection(new StructuredSelection(parentItem.getItem(nextIndex).getData()));
		return null;
	}
}
